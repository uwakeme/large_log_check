...
## [1.3.2] - 2026-09-11
### Changed
- **重新设计高级搜索为查询构建器** — 每个条件压缩为一行（字段 → 匹配方式 → 值），不再两级嵌套；AND/OR 逻辑改为条件行之间的「且/或」芯片，位置就在它生效的地方，点击即整体切换；线程/类/方法的匹配方式提升到行内第二列，默认从「精确」改为更常用的「包含」，关键词新增「包含任一词」模式（旧版只有全部词同时满足）；级别条件改为一键点选的彩色芯片（默认选中 ERROR/WARN），时间区间收进同一行、只填一头也算有效。删掉常驻提示框，示例改由各输入框 placeholder 承担；底部新增实时**查询预览**（如 `内容含全部词 "timeout" 且 级别为 ERROR/WARN`），搜索后同步常驻筛选状态栏、可一键取消，解决「搜完不知道生效中的是什么」。关闭弹窗不再清空已填条件（草稿保留，重开继续改，头部「清空」才重置）；新增 `Enter` 搜索、`Esc` 关闭快捷键，打开时自动聚焦第一个空输入框；未填写的条件在搜索时标红并跳过，结果提示中注明忽略数量。样式全部迁移至主题变量，4 套主题自动适配
### Added
