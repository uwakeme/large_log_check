...
- **搜索历史** - 搜索过的关键词自动记住，重新聚焦搜索框即可从历史下拉中一键重搜
...
**搜索历史：**
- 执行过的全局搜索都会自动记入历史，输入停顿后自动触发的搜索也算（多关键词整串、正则标记一并记录；「当前页」搜索与高级搜索不记录）
- 重新聚焦搜索框即弹出历史下拉，点击任一条目立即还原关键词与正则开关并重新搜索
- 下拉打开时继续输入可按子串实时过滤；正则条目带 `.*` 徽标
- 悬停条目点 × 可删除单条，底部「清空历史」一键清空；历史按（关键词 + 正则）去重、最新在前，打字过程中的中间前缀会自动合并进最终关键词，最多保留 50 条
- 历史按日志文件隔离（每个文件的查看器独立保存），仅当前会话有效，关闭查看器即清空

**高级搜索（查询构建器）：**
1. 点击「高级搜索」按钮，每个条件独占一行：**字段 → 匹配方式 → 值**，一目了然。支持的条件类型：
   - **关键词搜索**：多关键词用空格分隔，可选「包含全部词」（同时出现）或「包含任一词」（出现其一）
   - **线程名搜索**：包含（模糊）或精确等于线程名
   - **类名搜索**：包含（模糊）或精确等于类名（如 `com.example.UserService`）
   - **方法名搜索**：包含（模糊）或精确等于方法名
   - **日志级别过滤**：点选级别芯片（ERROR/WARN/INFO/DEBUG/其他），选中即点亮对应级别色
   - **时间范围过滤**：开始/结束时间填一头也算有效区间
2. 条件行之间的「且/或」芯片就是逻辑关系，点击即整体切换：
   - **且（AND）**：所有条件都必须满足
   - **或（OR）**：满足任一条件即可
3. 底部**查询预览**用一句话实时描述当前查询（如 `内容含全部词 "timeout" 且 级别为 ERROR/WARN`），确认无误再搜索；搜索后这句话会常驻筛选状态栏，随时提醒生效中的条件，可一键取消
4. 按 `Enter` 直接搜索、`Esc` 关闭弹窗；关闭不会丢失已填的条件，重开继续改，「清空」才重置
